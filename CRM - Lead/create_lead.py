...
def add_lead(contact_number:str, user_input:str):

    LeadSessionProxy.add_message(contact_number=contact_number, message={"role": "user", "content": user_input})
    messages = LeadSessionProxy.get_messages(contact_number=contact_number)
    ...
    if agent_id is None:
        print("No agent found for the contact number")
        return "No agent found for the contact number"

    crm_token = LeadProxy._get_crm_token_by_employee_database_id(employee_database_id=agent_id)
    # ADD Validation for the CRM Token
    if crm_token is None:
        print("No CRM Token found for the agent")
        return "No CRM Token found for the agent"
    ...
    if current_field is None and len(remaining_fields) == 0: #When all fields have been asked # Change the status of the lead to New.
        ...
        return "Lead created successfully in CRM"
        # else:
    ...
    LeadSessionProxy.add_message(contact_number=contact_number, message={"role": "assistant", "content": response.message_to_user})
    print(f"Assistant: {response.message_to_user}")
    return response.message_to_user


# Removed test code for production use