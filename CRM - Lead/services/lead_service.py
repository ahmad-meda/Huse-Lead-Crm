...
class LeadService:
    ...
    def _get_crm_token_by_employee_database_id(session: Session, employee_id: int) -> str:
        try:
            result = session.query(SalesTeam.crm_token).filter(
                SalesTeam.employee_id == employee_id
            ).first()  # Use first() instead of one() to avoid multiple rows error
            
            if result:
                return result[0] if isinstance(result, tuple) else result.crm_token
            return None

        except Exception as e:
            print(f"Error retrieving CRM token by employee ID: {str(e)}")
            return None
        
    @staticmethod
    def get_employee_name_by_crm_token(crm_token: str, db: Session):
        try:
            result = db.query(Employee.name).join(SalesTeam).filter(SalesTeam.crm_token == crm_token).first()
            if result:
                return result.name
            return None
        except Exception as e:
            print(f"Error retrieving employee name by CRM token: {str(e)}")
            return None
